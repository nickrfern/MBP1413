...
records = records.append({'metrics': (100,100),'train_acc':f'time{end-start}','val_acc': accuracy_score(y_test,y_predict),'matrix':matrix},ignore_index=True)
records.to_csv(f'{OUTPUT}mlp_realtest_records.csv')
np.save(f'/hpf/largeprojects/tabori/users/yuan/mbp1413/output/Figs/mlp_ypred',np.array(y_predict))
