...
records=records.append({'metrics':f'encoder_test_{variabels}','train_acc':'','val_acc':'','matrix':None},ignore_index=True)
records.to_csv(f'{OUTPUT}encoder_lgr_realtest_records.csv')
...
y_predict=lgr.predict(test_encoded)
np.save(f'/hpf/largeprojects/tabori/users/yuan/mbp1413/output/Figs/encoder_ypred',np.array(y_predict))
matrix = confusion_matrix(y_test,y_predict,labels=np.unique(y_test))
records = records.append({'metrics': variabels,'train_acc':'NA','val_acc': accuracy_score(y_test, lgr.predict(test_encoded)),'matrix':matrix},ignore_index=True)
records.to_csv(f'{OUTPUT}encoder_lgr_realtest_records.csv')